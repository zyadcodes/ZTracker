...
export default class historyScreen extends Component {
  ...
  async componentDidMount() {
    const { userID, allMonths } = this.props.navigation.state.params;
    allMonths.reverse();
    //Constructs the data
    const tableData = [];
    for (const month of allMonths) {
      const monthData = [
        ...
        this.getTouchableOpacity('$' + month.revenue.toFixed(2), () => {
          this.props.navigation.push('SpecificMonthScreen', {
            month: month,
            userID
          });
        }),
        this.getTouchableOpacity('$' + month.expenses.toFixed(2), () => {
          this.props.navigation.push('SpecificMonthScreen', {
            month: month,
            userID
          });
        }),
        this.getTouchableOpacity('$' + month.profit.toFixed(2), () => {
          this.props.navigation.push('SpecificMonthScreen', {
            month: month,
            userID
          });
